...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=2, os=16, pretrained=False, _print=True):
        ...
        # self.decode = nn.Conv2d(n_classes,n_classes,kernel_size=3,stride=1,padding=1, bias=False)
        self.resnet_features_dropout = nn.Dropout2d(p = 0.5)
        self.concat_dropout = nn.Dropout2d(p = 0.5)

    def forward(self, input):
        x, low_level_features = self.resnet_features(input)
        x = self.resnet_features_dropout(x)
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
